defmodule SMPPEX.Protocol.UnpackTest do
  ...
  test "c_octet_string: fixed, hex" do
    assert {:ok, "0123456789abcdefABCDEF", "c"} == Unpack.c_octet_string(<<"0123456789abcdefABCDEF", 0, "c">>, {:fixed, 23}, :hex)
    assert {:error, _} = Unpack.c_octet_string(<<"0123456789abXdefABCDEF", 0, "c">>, {:fixed, 23}, :hex)
  end

  test "c_octet_string: fixed, dec" do
    assert {:ok, "0123456789", "c"} == Unpack.c_octet_string(<<"0123456789", 0, "c">>, {:fixed, 11}, :dec)
  ...
  test "c_octet_string: var max, hex" do
    assert {:ok, "0123456789abcdefABCDEF", "c"} == Unpack.c_octet_string(<<"0123456789abcdefABCDEF", 0, "c">>, {:max, 23}, :hex)
    assert {:error, _} = Unpack.c_octet_string(<<"0123456789abXdefABCDEF", 0, "c">>, {:max, 23}, :hex)
  end

  test "c_octet_string: var max, dec" do
    assert {:ok, "0123456789", "c"} == Unpack.c_octet_string(<<"0123456789", 0, "c">>, {:max, 11}, :dec)
